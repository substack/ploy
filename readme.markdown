...
usage:
  ...
  ploy work { -r REMOTE | --format=FORMAT }
    ...
    Directories with a branch listed are currently active.
    The default FORMAT is `commit,time,branch`.

  ploy clean

    Remove unused working directories. These are the directories in `ploy work`
    without a branch.
